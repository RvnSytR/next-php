...
$configFile = $docRoot . "/src/json/config.json";
if (!file_exists($configFile)) {
    file_put_contents($configFile, json_encode($defaultConfig, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
}

$configJson = file_get_contents($configFile);
if (empty(trim($configJson))) $config = [];
else {
    $config = json_decode($configJson, true);
    if (json_last_error() !== JSON_ERROR_NONE) {
        header("Content-Type: application/json");
        responseError(new Error("Gagal memuat konfigurasi. Silakan periksa format JSON pada config.json.", 500));
    };
}
