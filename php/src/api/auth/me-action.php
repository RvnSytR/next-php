...
action("POST", function ($req, $db) {
    ...
    $id = $_SESSION["id"];
    $currentPassword = $db["user"]["select-password-by-id"]($id)->fetch_assoc()[
        "password"
    ];
    ...
    if (password_verify($data["newPassword"], $currentPassword)) {
        throw new Error(
            "Tidak ada perubahan yang dilakukan pada kata sandi.",
            409,
        );
    }

    $db["user"]["update-password-by-id"](
        $id,
        password_hash(trim($data["newPassword"]), PASSWORD_BCRYPT),
    );
...
action("DELETE", function ($req, $db) {
    if (isset($_SESSION["image"])) {
        ...
        responseError(new Error("Tidak ada foto profil yang diunggah.", 400));
    }
